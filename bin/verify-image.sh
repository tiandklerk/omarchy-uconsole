...
# The build chroots need a working resolv.conf to reach the mirrors, so the
# resolv.conf from the build container gets copied in. Shipping it leaves the
# device unable to resolve anything despite having a working wifi connection.
! grep -qi "docker\|Generated by Docker Engine" /m/etc/resolv.conf 2>/dev/null
chk "no build-host DNS config leaked into the image" $?
[[ -L /m/etc/resolv.conf ]]
